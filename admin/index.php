            ...
            <div class="row">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        $db = Database::connect(); #fonction connect de la classe Database du fichier database.php
                        #récuperer les résultats de notre requete via SQL
                        $statement = $db->query('SELECT items.id, items.name, items.description, items.price, categories.name AS category
                                                    FROM items LEFT JOIN categories ON items.category = categories.id 
                                                    ORDER BY items.id DESC'); #ordre décroissant
                        while($item = $statement->fetch()){ #les afficher
                            echo '<tr>';
                               echo '<td>' . $item['name'] . '</td>'; #concatenation avec le nom de la colonne sur phpmyadmin
